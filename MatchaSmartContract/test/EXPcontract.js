...
describe('Expense', function () {
  async function deployContract() {
    ...
    //deploye Expense
    const Expense = await ethers.getContractFactory('SimpliSpend');
  ...
  describe('testing', function () {
    ...
    it('owner is correct', async function () {
      const { hardhatExpense, owner, addr1 } = await loadFixture(
        deployContract
      );
    ...
    it('owner mint tokens', async function () {
      const { hardhatExpense, owner, addr1, mockUSDC } = await loadFixture(
        deployContract
      );
    ...
    it('User can put USDC inside the contract and owner can swap EXP for USDC', async function () {
      ...
      await mockUSDC
        .connect(addr1)
        .approve(hardhatExpense.address, ethers.utils.parseUnits('50000', 6));
      await hardhatExpense
        .connect(addr1)
        .fundUSDC(ethers.utils.parseUnits('50000', 6));
      ...
      expect(await mockUSDC.balanceOf(hardhatExpense.address)).to.equal(
      ...
      await hardhatExpense
        .connect(owner)
        .exchangeEXP(ethers.utils.parseUnits('50000', 6));
